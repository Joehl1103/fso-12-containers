...
void (0, node_test_1.test)('patient list does not include entries', () => __awaiter(void 0, void 0, void 0, function* () {
    const response = yield (0, supertest_1.default)(app_1.default)
        .get('/api/patients')
        .expect(200);
    const patients = response.body;
    node_assert_1.default.ok(patients.length > 0, 'Expected at least one patient in the response');
    for (const patient of patients) {
        node_assert_1.default.strictEqual(patient.entries, undefined, `Patient ${patient.name} should not have entries in list response`);
        node_assert_1.default.strictEqual(patient.ssn, undefined, `Patient ${patient.name} should not have ssn in list response`);
        node_assert_1.default.ok(patient.id, 'Patient should have id');
        node_assert_1.default.ok(patient.name, 'Patient should have name');
        node_assert_1.default.ok(patient.gender, 'Patient should have gender');
        node_assert_1.default.ok(patient.occupation, 'Patient should have occupation');
        node_assert_1.default.ok(patient.dateOfBirth, 'Patient should have dateOfBirth');
    }
}));
